...
# -*- coding: utf-8 -*-
"""
Created on Sun Aug 20 12:13:46 2017
...
plt.title('Naive based Classification (training set')
plt.xlabel('age')
...
for i,j in enumerate(np.unique(Y_set)):
    plt.scatter(X_set[Y_set == j,0], X_set[Y_set == j,1],
                c = ListedColormap(('red', 'green'))(i), label=j)
plt.title('Naive Based Classification (test set')
plt.xlabel('age')
plt.ylabel('Estimated Salary')
plt.legend()
plt.show()